    ...
    return (
        <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-[#181818] via-[#232323] to-[#181818] relative overflow-hidden '>
            ...
            {!(recommendation && recommendation.length > 0) && (
                <img src={banner} alt="netflex banner bg" className='absolute inset-0 w-full h-full object-cover opacity-20 blur-[2px]' />
            )}


            {recommendation && recommendation.length > 0 ? (
                <div className="w-full max-w-7xl mx-auto mt-2">
                    <h2 className="text-2xl font-bold text-bold text-white mb-4 text-center">AI Recommended Movies</h2>
                    <RecommendedMovies moviesTitles={recommendation} />
                </div>
            ) :
                <div className='relative w-full max-w-md mx-auto rounded-2xl bg-[#181818]/90 shadow-2xl border border-[#333] px-8 py-10 mt-4 flex flex-col items-center min-h-[480px] mb-5'>
                    <h2 className='text-3xl font-extrabold mb-8 text-center text-white tracking-tight drop-shadow-lg'>AI Movie Recommendation</h2>
                        ...
                        <div className="mb-6 flex-1">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {steps[step].options.map(opt => (
                                    <button key={opt} onClick={() => handleOptions(opt)} className={`w-full py-3 cursor-pointer rounded-xl border-2 transition font-semibold text-base flex items-center justify-center gap-2 focus:outline-none focus:ring-2 active:scale-95 duration-150 focus:ring-[#e50914] ${inputs[steps[step].name] == opt ? "bg-[#e50914] border-[#e50914] text-white shadow-lg" : "bg-[#232323] border-[#444] text-white hover:bg-[#e50914]/80 hover:border-[#e50914]"} text-white haove:bg-[#e50914]/80`}>{opt}</button>
                                ))}
                            </div>
                        </div>
                        <div className="flex justify-between items-center mt-6">
                            <button className='px-6 py-2 rounded-lg font-semibold transition border-2 border-[#444] text-white bg-[#181818] hover:bg-[#232323]' type='button' onClick={handleBack} disabled={step == 0}>Back</button>
                            <button className='px-6 py-2 rounded-lg font-semibold transition border-2 border-[#444] text-white bg-[#e50914] hover:bg-[#b0060f] ml-2' type='button' onClick={step === steps.length - 1 ? generateRecommendations : handleNext} disabled={!inputs[steps[step].name] || isLoading}>
